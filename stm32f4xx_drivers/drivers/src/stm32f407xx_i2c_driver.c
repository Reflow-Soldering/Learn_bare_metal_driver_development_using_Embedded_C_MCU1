...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle)
{
	...
	//1. Handle For interrupt generated by SB event -> SB 이벤트 발생 시 처리하는 루틴
	//	Note : SB flag is only applicable in Master mode -> SB 플래그는 마스터에서만 작동함
	if(temp1 && temp3)
	{
	...
	//BTF 플래그 set은 SR/DR이 동시에 EMPTY(TX일 때)거나 동시에 FULL(RX일 때)이다.
	if(temp1 && temp3)
	{
		//BTF flag is set
		if(pI2CHandle->TxRxState == I2C_BUSY_IN_TX)
